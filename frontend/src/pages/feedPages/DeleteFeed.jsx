...
const DeleteFeed = () => {
  ...
  const handleDeleteFeed = () => {
    setLoading(true);
    axios
      .delete(`http://localhost:5555/feed/${id}`)
      .then(() => {
        setLoading(false);
        navigate("/feed");
      })
  ...
  return (
    <>
      ...
      >
        <h1 className="fs-1 fw-bold my-4 text-white mb-5">Delete Feeding</h1>
        {loading ? <LoadSpinner /> : ""}
        <div className="d-flex align-items-center mx-auto text-michgold">
          <h3>Are You Sure You Want To Delete This Feeding?</h3>
        </div>
        <div>
          <Button className="btn-danger m-3" onClick={handleDeleteFeed}>
            YES
          </Button>
          <BackButton />
        </div>
